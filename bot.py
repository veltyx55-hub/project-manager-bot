...
async def mark_done(interaction: discord.Interaction, role: str, chapter: Optional[str] = None):
    ...
    async with pool.acquire() as conn:
        if chapter:
            ...
            if len(active_rows) > 1:
                ...
                return await interaction.response.send_message(
                    f"📋 Kamu punya beberapa chapter **{role}** aktif: **Ch {ch_list}**\n"
    ...
    if role in TL_ROLES:
        ...
        # kalau TS belum ada / belum diambil → auto create
        if not ts_row or ts_row["status"] != "claimed":
            ...
            await interaction.channel.send(
                f"📢 TS sudah ada untuk Ch {chapter} → <@{ts_row['assignee_id']}>"
            )
...
async def deadline_check():
    ...
    for r in rows:
        ...
        if ch:
            ...
            if OWNER_ID:
                notice += f"\n🔔 <@{OWNER_ID}> perlu reauction **{r['role']} Ch {r['chapter']}**"
